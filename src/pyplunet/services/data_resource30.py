...
class DataResource30:
    ...
    def update(
        ...
        if type(resource_in) is not ResourceIN:
    ...
    def insert(self, working_status: Union[WorkingStatus, int]) -> IntegerResult:
        ...
        if type(working_status) is WorkingStatus:
            working_status = working_status.value
        elif type(working_status) is int:
    ...
    def search(
        ...
        if type(search_filter_resource) is not SearchFilter_Resource:
    ...
    def set_payment_information(
        ...
        if type(payment_info) is not PaymentInfo:
    ...
    def set_resource_type(
        ...
        if type(resource_type) is ResourceType:
            resource_type = resource_type.value
        elif type(resource_type) is int:
